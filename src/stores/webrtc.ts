...
export class PeerMedia {
  ...
  audioSrcNode?: MediaStreamAudioSourceNode
  volumeWorkletNode?: AudioWorkletNode
  volume: number = 0
        ...
        peerMedias.value[remotePeerId].mediaConn?.on(
          ...
          async (remoteStream: MediaStream) => {
            ...
            peerMedias.value[remotePeerId].volumeWorkletNode = new AudioWorkletNode(
              peerMedias.value[remotePeerId].audioContext as AudioContext,
              'volume-meter'
            )
            ;(peerMedias.value[remotePeerId].volumeWorkletNode as AudioWorkletNode).port.onmessage =
              ({ data }) => {
                peerMedias.value[remotePeerId].volume = data * 500
              }
            peerMedias.value[remotePeerId].audioSrcNode?.connect(
              peerMedias.value[remotePeerId].volumeWorkletNode as AudioWorkletNode
            )
          }
        )
